...
const addExclamation = (arr) => {
  // Solution code here...
  const emphasisAddedArray = [];
  arr.forEach((string) => {
    emphasisAddedArray.push(string + '!');
  });

  return emphasisAddedArray;
};
...
const allUpperCase = (arr) => {
  ...
  arr.forEach((lowercase) => {
    upperCaseArray.push(lowercase.toUpperCase());
  });
...
const speaker = (words, callback) => {
  // Solution code here...
  const callbackedArray = [];
  words.forEach((string) => {
    callbackedArray.push(callback(string));
  });
