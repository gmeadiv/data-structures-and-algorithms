...
const addTwo = (arr) => {
  ...
  for (let i=0; i<arr.length; i++) {
    twoAddedArray.push(arr[i] + 2);
  }

  return twoAddedArray;
};
...
const typeNum = (arr) => {
  // Solution code here...
  const numbersOnlyArray = arr.filter((number) => {
...
const oddValues = (arr) => {
  // Solution code here...
  const oddsOnlyArray = arr.filter((number) => {
    if (!(number%2)) {} else return number;
  });
