...
const toLastNames = people => {
  ...
  const getFullName = people => {
    return [people.firstName, people.lastName].join(" ");
  };
  const fullNameArray = people.map(getFullName);
...
const addValues = (arr) => {
  // Solution code here...
  const sum = arr.reduce((number, value) => {
    number = number + value;
    return number;
  }, 0);
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const elementsCounter = arr.reduce( (number) => {
    return number + 1;
  });
...
const reversedString = (str) => {
  ...
  const reversedArray = stringArray.reduce((reversed, string) => {
